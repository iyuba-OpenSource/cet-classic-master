...
public class AttentionCenter extends BasisActivity implements
    ...
    private ArrayList<Attention231219.DataDTO> attentionArrayList = new ArrayList<>();
...
//							}
                                ...
                                public void finish(BaseHttpResponse bhr) {
                                    ...
                                    ResponseAttentionList res = (ResponseAttentionList) bhr;
                                    if (res.attention231219.getResult().equals("550")) {
                                        attentionArrayList.addAll(res.attention231219.getData());
                                        adapter.setData(attentionArrayList);
                                        if (attentionArrayList.size() >= res.attention231219.getNum()) {
                                            isLastPage = true;
                                        }
                                    } else {
                                    }
        ...
        fansList.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int position, long arg3) {
                ...
                SocialDataManager.Instance().attention = attentionArrayList
                        .get(position);
                Attention231219.DataDTO attention = attentionArrayList.get(position);
                String username = attention.getFusername();
                String uid = attention.getFollowuid();
