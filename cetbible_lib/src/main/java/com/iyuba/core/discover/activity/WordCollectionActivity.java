...
public class WordCollectionActivity extends BasisActivity implements
                ...
                case 9:
                    ...
                    ClientSession.Instance().asynGetResponse(
                            new WordUpdateRequest(userId,
                                    ...
                                    word.key), new IResponseReceiver() {
...
//                                    wo.deleteItemWord(userId);
                                    ...
                                    WordUpdateResponse wordUpdateResponse = (WordUpdateResponse) response;
                                    wo.tryToDeleteItemWord(wordUpdateResponse.word, userId);

                                    List<Word> wlist = wordListAdapter.getmList();
                                    for (int i = 0; i < wlist.size(); i++) {

                                        Word twWord = wlist.get(i);
                                        if (twWord.key.equalsIgnoreCase(word.key)) {

                                            wlist.remove(i);
                                            runOnUiThread(() -> wordListAdapter.notifyDataSetChanged());
                                            break;
                                        }
                                    }
                                }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView button_delete = findViewById(R.id.button_delete);
    ...
    private void onResume0() {
        ...
        if (wordListView != null) {
           ...
           /* tryToDeleteWords = (ArrayList<Word>) wo.findDataByDelete(userId);
            ...
            wordListAdapter.setCallback(new WordListAdapter.Callback() {
                @Override
