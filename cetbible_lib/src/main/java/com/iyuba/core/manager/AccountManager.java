...
public class AccountManager {
    ...
    private void initCommonConstants() {
        ...
        if (AccountManager.Instace(mContext.getApplicationContext()).checkUserLogin()) {
...
//        PersonalManager.getInstance().setUserId(user.uid);
    ...
    public void getUserInfoAndLogin() {
        ...
        userId = ConfigManager.Instance().loadString("userId", "0");
        userInfo = new UserInfoOp(mContext).selectData(userId);
    ...
    private void setUserStatus(UserInfo userInfo) {
        ...
        PersonalHome.setSaveUserinfo(Integer.parseInt(userId), userName, userInfo.vipStatus);
