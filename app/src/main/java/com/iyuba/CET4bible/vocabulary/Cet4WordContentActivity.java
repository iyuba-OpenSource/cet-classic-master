 ...
 * 单词详情（词汇大全）
 */
public class Cet4WordContentActivity extends BasisActivity implements
    ...
    private void setContent() {
        ...
        if (mode.getCheckedRadioButtonId() == R.id.study) {
            ...
            if (wordOp.findDataByName(cet4Word.word) != null) {
                addWord2.setBackgroundResource(com.iyuba.biblelib.R.drawable.word_add_ok);
            } else {
                addWord2.setBackgroundResource(com.iyuba.biblelib.R.drawable.word_add);
            }
        } else {
            word.setText(cet4Word.word);
            if (wordOp.findDataByName(cet4Word.word) != null) {
                addWord.setBackgroundResource(com.iyuba.biblelib.R.drawable.word_add_ok);
            } else {
                addWord.setBackgroundResource(com.iyuba.biblelib.R.drawable.word_add);
            }
        }
    }
    ...
    private void saveNewWords() {
        if (!AccountManager.Instace(mContext).checkUserLogin()) {
            ...
            CustomToast.showToast(mContext, com.iyuba.biblelib.R.string.play_ins_new_word_success);
         /*   if (BuildConfig.isEnglish) {
    ...
    public void onClick(View arg0) {
        ...
        if (id == R.id.button_back) {
            ...
            toNextOne();
        } else if (id == R.id.word_add || id == R.id.word_add2) {
            saveNewWords();
        } else if (id == R.id.so_easy) {
            vv.pause();
            try {
